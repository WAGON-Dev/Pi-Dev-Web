...
use GoVoyageBundle\Entity\Demande;
...
class DemandeController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        $demandes = $em->getRepository('GoVoyageBundle:Demande')->findAll();
    ...
    {
        $demande = new Demande();
        $form = $this->createForm('GoVoyageBundle\Form\DemandeType', $demande);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->redirectToRoute('demande_show', array('idVpFk' => $demande->getIdvpfk()));
        }
    ...
    {
        $deleteForm = $this->createDeleteForm($demande);
        $editForm = $this->createForm('GoVoyageBundle\Form\DemandeType', $demande);
        $editForm->handleRequest($request);

        if ($editForm->isSubmitted() && $editForm->isValid()) {
            $this->getDoctrine()->getManager()->flush();

            return $this->redirectToRoute('demande_edit', array('idVpFk' => $demande->getIdvpfk()));
        }
    ...
    private function createDeleteForm(Demande $demande)
    {
        return $this->createFormBuilder()
            ->setAction($this->generateUrl('demande_delete', array('idVpFk' => $demande->getIdvpfk())))
            ->setMethod('DELETE')
            ->getForm()
        ;
    }
